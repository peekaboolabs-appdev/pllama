...
Future<int> pllamaInference(
  ...
  try {
    ...
    // ignore: avoid_print
    print('[pllama] ERROR sending request to helper isolate: $e');
  }
  return requestId;
}
...
Pointer<pllama_inference_request> _toNative(
    FllamaInferenceRequest dart, int requestId) {
  // Allocate memory for the request structure.
  final Pointer<pllama_inference_request> requestPointer =
      calloc<pllama_inference_request>();
  final pllama_inference_request request = requestPointer.ref;
...
Future<SendPort> _helperIsolateSendPort = () async {
  ...
  await Isolate.spawn(_pllamaInferenceIsolate, receivePort.sendPort);
...
void pllamaCancelInference(int requestId) async {
  final SendPort helperIsolateSendPort = await _helperIsolateSendPort;
...
void _pllamaInferenceIsolate(SendPort sendPort) async {
  final ReceivePort helperReceivePort = ReceivePort();
  helperReceivePort.listen((dynamic data) {
    try {
      if (data is _IsolateInferenceCancel) {
        pllamaBindings.pllama_inference_cancel(data.id);
        return;
      }
      ...
      pllamaBindings.pllama_inference(
        nativeRequest,
        callback.nativeFunction,
      );
    } catch (e, s) {
      // ignore: avoid_print
      print('[pllama inference isolate] ERROR: $e. STACK: $s');
    }
  });
