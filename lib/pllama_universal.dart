...
import 'package:pllama/misc/gbnf.dart';
import 'package:jinja/jinja.dart';

import 'model/model_override.dart';

/// Returns true if the given [output] indicates that the model failed to load.
/// Output is the output from [pllamaInference] or [pllamaChat].
bool pllamaOutputIndicatesLoadError(String output) {
...
/// Parameters needed to run standard LLM inference. Use with [pllamaInference].
///
...
/// is run. Use with [pllamaTokenize].
...
Future<int> pllamaChat(
  ...
  chatTemplate = pllamaSanitizeChatTemplate(
      await pllamaChatTemplateGet(request.modelPath), request.modelPath);
  eosToken = chatTemplate == chatMlTemplate
      ? chatMlEosToken
      : await pllamaEosTokenGet(request.modelPath);
  bosToken = chatTemplate == chatMlTemplate
      ? chatMlBosToken
      : await pllamaBosTokenGet(request.modelPath);

  text = pllamaApplyChatTemplate(
  ...
  if (request.tools.isNotEmpty) {
    if (request.tools.length > 1) {
      // ignore: avoid_print
      print(
          '[pllama] WARNING: More than one tool was specified. No grammar will be enforced. (via pllamaChat)');
      grammar = '';
    } else {
      grammar = request.tools.first.grammar;
      // ignore: avoid_print
      print('[pllama] Grammar to be enforced: $grammar');
    }
  } else {
    // ignore: avoid_print
    print('[pllama] No tools were specified. No grammar will be enforced.');
    grammar = '';
  }
  ...
  return pllamaInference(inferenceRequest, callback);
}
...
String pllamaApplyChatTemplate({
...
''',
  ...
  final globals = <String, Function>{
    'raise_exception': (String message) {
      // ignore: avoid_print
      print('[pllama] chat template asked to raise_exception: $message');
      return '';
    }
  };
  ...
  try {
    ...
    // ignore: avoid_print
    print('[pllama] Error applying chat template: $e');
    // ignore: avoid_print
    print('[pllama] chat template: $chatTemplate');
    // ignore: avoid_print
    print('[pllama] messages: $jsonMessages');
    if (chatTemplate != chatMlTemplate) {
      ...
      if (llamaChatTemplate.matches(chatTemplate)) {
        ...
        print(
            '[pllama] Using Llama 3 chat template as a fallback because the chat template could not be applied. Exception: $e. Chat template: $chatTemplate. Messages: $jsonMessages.');
        return pllamaApplyChatTemplate(
      ...
      print(
          '[pllama] Using ChatML because the chat template could not be applied. Exception: $e. Chat template: $chatTemplate. Messages: $jsonMessages.');
      return pllamaApplyChatTemplate(
      ...
      // ignore: avoid_print
      print(
          '[pllama] Exception thrown while applying chat template. ChatML could not be used as a fallback. Returning empty string. Exception: $e. Chat template: $chatTemplate. Messages: $jsonMessages.');
      return '';
    }
  }
}
...
String pllamaJsonSchemaToGrammar(String jsonSchema) {
...
String pllamaSanitizeChatTemplate(
  ...
  if (builtInChatTemplate
    ...
    // ignore: avoid_print
    print(
        '[pllama] Using ChatML because built-in chat template seems erroneous. (contains "Only user and assistant roles are supported!")');
    ...
    // ignore: avoid_print
    print('[pllama] Using built-in chat template: $chatTemplate');
    // ignore: avoid_print
  } else {
    // Assume models without one specified intend ChatML.
    // This is the case for Mistral 7B via OpenHermes.
    // ignore: avoid_print
    print('[pllama] Using ChatML because no built-in chat template was found.');
    chatTemplate = chatMlTemplate;
  }
  return chatTemplate;
}
